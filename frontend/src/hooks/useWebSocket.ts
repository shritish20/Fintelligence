...
export function useWebSocket(url: string, options: WebSocketOptions = {}) {
  ...
  const connect = useCallback(() => {
    // Read JWT from stored user object
    let jwt: string | null = null
    try {
      const raw = localStorage.getItem('fintelligence_user')
      if (raw) jwt = JSON.parse(raw)?.accessToken ?? null
    } catch { /* ignore */ }

    // Pass JWT as query param for WebSocket auth
    const fullUrl = jwt && !url.includes('token=')
      ? `${url}${url.includes('?') ? '&' : '?'}token=${jwt}`
      : url;
