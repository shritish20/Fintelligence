...
# ── Token encryption removed (Option B) ──────────────────────────────────────
# Broker tokens are never stored on the server (DB or memory between requests).
# They live only in the user's device sessionStorage and are sent per-request
# via the X-Upstox-Token header. No encryption needed — nothing to encrypt.
# TOKEN_ENCRYPTION_KEY env var is no longer required.
...
# ============================================================================
# DATABASE BOOTSTRAP — PostgreSQL via shared db_utils
# ============================================================================
from sqlalchemy.orm import declarative_base as _declarative_base
from sqlalchemy.orm import sessionmaker as _sessionmaker
from contextlib import contextmanager as _cm
from db_utils import make_engine as _make_engine, make_redis as _make_redis

Base = _declarative_base()

# Engine created eagerly — PostgreSQL doesn't need lazy init like SQLite did
engine       = _make_engine(schema_prefix="volguard")
SessionLocal = _sessionmaker(bind=engine, expire_on_commit=False)

# Redis client (DB index 0 = volguard)
redis_client = _make_redis(db=0)

def _init_db(database_url: str):
    """Legacy stub — kept so call sites below don't break. Engine already created above."""
    # TODO: This stub can be removed once all _init_db() call sites are cleaned up.
    pass
...
class DynamicConfig:
        ...
        with cls._lock:
            ...
            try:
                ...
                for key, default_val in cls.DEFAULTS.items():
                    result = db.execute(
                        text("SELECT value FROM dynamic_config WHERE key = :key AND user_id IS NULL"),
                        {"key": key}
                    ).fetchone()
                    ...
                    if result:
                        ...
                        try:
                            ...
                            logger.exception("Unexpected error — check traceback above for details")
    ...
    @classmethod
    def _persist(cls, key, value, db=None, user_id: int = None):
        close_db = False
        if db is None:
        ...
        try:
            ...
            if user_id is not None:
                db.execute(
                    text("""
                        INSERT INTO dynamic_config (key, value, user_id, updated_at) 
                        VALUES (:key, :value, :user_id, :updated_at)
                        ON CONFLICT(key, user_id) DO UPDATE SET 
                        value = excluded.value, 
                        updated_at = excluded.updated_at
                    """),
                    {"key": key, "value": str_val, "user_id": user_id, "updated_at": datetime.now()}
                )
            else:
                # PostgreSQL treats NULLs as distinct in unique indexes, so
                # ON CONFLICT (key, user_id) won't fire when user_id IS NULL.
                # Use an explicit check-then-insert/update pattern instead.
                existing = db.execute(
                    text("SELECT id FROM dynamic_config WHERE key = :key AND user_id IS NULL"),
                    {"key": key}
                ).fetchone()
                if existing:
                    db.execute(
    ...
    @classmethod
    def get(cls, key, default=None, user_id: int = None):
        """Get config value. Per-user override takes priority over global."""
        with cls._lock:
            if user_id is not None:
                user_key = f"user_{user_id}_{key}"
                if user_key in cls._values:
                    return cls._values[user_key]
            return cls._values.get(key, default)

    @classmethod
    def get_for_user(cls, user_id: int) -> dict:
        """Return full config dict merged: global defaults overridden by user's saved values."""
        with cls._lock:
            merged = {k: v for k, v in cls._values.items() if not k.startswith("user_")}
    ...
    @classmethod
    def load_user_overrides(cls, user_id: int):
        """Load a user's saved config overrides from DB into memory."""
        if not cls._db_session_factory:
            return
        db = cls._db_session_factory()
        try:
            rows = db.execute(
                text("SELECT key, value FROM dynamic_config WHERE user_id = :uid"),
                {"uid": user_id}
            ).fetchall()
            with cls._lock:
                for row in rows:
                    key, stored = row[0], row[1]
                    if key in cls.DEFAULTS:
                        default_type = type(cls.DEFAULTS[key])
                        try:
                            if default_type == bool:
                                val = stored.lower() == 'true'
                                ...
                                val = stored
                            cls._values[f"user_{user_id}_{key}"] = val
                        except Exception:
                            logger.debug(f"Could not cast config value for key={key}", exc_info=True)
        finally:
            db.close()

    @classmethod
    def update(cls, updates: dict, user_id: int = None):
        """Update config. Per-user if user_id given, global otherwise (admin only)."""
        with cls._lock:
                ...
                for key, new_val in updates.items():
                    if key in cls.DEFAULTS:
                        ...
                        try:
                            ...
                            store_key = f"user_{user_id}_{key}" if user_id else key
                            cls._values[store_key] = new_val
                            cls._persist(key, new_val, db, user_id=user_id)
                            changed[key] = new_val
                            logger.info(f"Config updated: {key} = {new_val} (user_id={user_id})")
                        except Exception as e:
                            logger.exception(f"Failed to update {key}")
    ...
    @classmethod
    def to_dict(cls, user_id: int = None):
        if user_id is not None:
            return cls.get_for_user(user_id)
        with cls._lock:
            return {k: v for k, v in cls._values.items() if not k.startswith("user_")}
...
class User(Base):
    ...
    # NOTE (Option B): upstox_token intentionally NOT stored here.
    # The broker token lives only on the user's device (sessionStorage).
    # The server never receives or persists it. This is your competitive moat.
    ...
    last_invalidated_at = Column(DateTime, nullable=True)          # set to revoke all tokens


class AdminAuditLog(Base):
    """Immutable record of every admin action. Never deleted."""
...
class SubscriptionPayment(Base):
    """Razorpay payment records — one row per checkout attempt."""
...
def _get_user_from_jwt(
    ...
    if not credentials:
        raise HTTPException(status_code=401, detail="Not authenticated — provide Bearer token")
    ...
    # Soft revocation check — if admin invalidated this user's tokens,
    # reject tokens issued before the invalidation timestamp
    if user.last_invalidated_at:
        token_exp = payload.get("exp", 0)
        token_issued_approx = token_exp - (JWT_EXPIRE_DAYS * 86400)
        invalidated_ts = user.last_invalidated_at.timestamp()
        if token_issued_approx < invalidated_ts:
...
def get_current_user(
    ...
    """Dependency — returns the authenticated User object."""
    return _get_user_from_jwt(credentials, db)


async def verify_token(
    credentials: HTTPAuthorizationCredentials = Security(_bearer_scheme),
    x_upstox_token: Optional[str] = Header(None, alias="X-Upstox-Token"),
    db: Session = Depends(get_db),
) -> str:
    """
    TRADING dependency — validates JWT and returns the Upstox broker token.

    Option B: The broker token is NOT stored on the server. The client sends it
    on every trading request via the X-Upstox-Token header. It lives only in
    the device's sessionStorage and is never persisted server-side.

    Raises 403 if the client did not send the header.
    """
    _get_user_from_jwt(credentials, db)   # validates JWT, raises 401 on failure
    if not x_upstox_token or len(x_upstox_token.strip()) < 20:
        raise HTTPException(
            status_code=403,
            detail={
                "code": "NO_TRADING_TOKEN",
                "message": "Paste your daily Upstox token to activate live trading features. "
                           "It is never stored on the server — send it via X-Upstox-Token header."
            }
        )
    # TODO: For multi-user SaaS, consider tracking which user_id maps to which
    # in-memory SDK session via volguard_system.active_user_id
    return x_upstox_token.strip()
...
async def jwt_required(
    ...
    """
    ANALYTICS dependency — validates JWT only. No Upstox token required.
    Used by intelligence, dashboard, news, regime, journal, system endpoints.
    Anyone logged in can access these — Upstox token is NOT required.
    """
    return _get_user_from_jwt(credentials, db)


def require_admin(current_user: "User" = Depends(jwt_required)) -> "User":
    """Raises 403 if the requesting user is not an admin.
    Protects system config, emergency exit, and log endpoints."""
    if not current_user.is_admin:
        raise HTTPException(
            status_code=403,
            detail="Admin access required. Contact the system owner."
        )
    return current_user
...
class WebSocketManager:
    ...
    async def send_message(self, message: dict, websocket: WebSocket):
        try:
            ...
            self.logger.exception(f"Error sending message")
...
class SmartDataFetcher:
    ...
    def get_ltp(self, instrument_key: str) -> Optional[float]:
        ...
        try:
            ...
            self.logger.exception(f"REST API LTP fallback failed for {instrument_key}")
    ...
    def get_bulk_ltp(self, instrument_keys: List[str]) -> Dict[str, Optional[float]]:
        ...
        if missing:
            try:
                ...
                self.logger.exception(f"Bulk REST LTP failed")
    ...
    def get_ohlc(self, instrument_key: str, interval: str = "1d") -> Optional[Dict]:
        try:
            ...
            self.logger.exception(f"OHLC fetch failed for {instrument_key}")
    ...
    def get_full_quote(self, instrument_key: str) -> Optional[Dict]:
        try:
            ...
            self.logger.exception(f"Full quote fetch failed for {instrument_key}")
...
class CorrelationManager:
    ...
    def can_take_position(self, proposed_strategy: ConstructedStrategy, user_id: Optional[int] = None) -> Tuple[bool, List[CorrelationViolation]]:
        db = self.db_session_factory()
        try:
            q = db.query(TradeJournal).filter(
                TradeJournal.status == TradeStatus.ACTIVE.value
            )
            if user_id is not None:
                q = q.filter(
                    (TradeJournal.user_id == user_id) | (TradeJournal.user_id == None)
                )
            active_trades = q.all()
            ...
            for rule in self.rules:
                ...
                for trade in active_trades:
                    try:
                        ...
                        self.logger.exception(f"Error checking trade {trade.strategy_id}")
                        continue
    ...
    def get_correlation_report(self, user_id: Optional[int] = None) -> Dict:
...
class V5NewsScanner:
    ...
    def _fetch_feed_with_timeout(url: str, timeout: int = 8):
        """Fetch RSS feed safely using requests with per-request timeout.
        Avoids socket.setdefaulttimeout() which is a global process-level setting
        and is NOT thread-safe when called from concurrent ThreadPoolExecutor workers.
        """
        try:
            ...
            resp = _req.get(
                url,
                timeout=timeout,
                headers={"User-Agent": "Fintelligence/1.0 RSS Reader"},
            )
            resp.raise_for_status()
            return feedparser.parse(resp.content)
        except Exception:
            # Return an empty parse result so the caller's error handling works
...
def _v5_parse_brief(raw: str) -> V5BriefResult:
    ...
    try:
        ...
        logger.exception(f"parse_brief error")
    return r
...
def _v5_parse_pretrade(raw: str) -> V5PreTradeResult:
    ...
    try:
        ...
        logger.exception(f"parse_pretrade error")
    return r
...
def _v5_parse_alert(raw: str) -> V5AlertResult:
    ...
    try:
        ...
        logger.exception(f"parse_alert error")
    return r
...
class V5MorningBriefAgent:
    ...
    def _save_to_db(self, result: V5BriefResult):
        try:
            ...
            self.logger.exception(f"DB save brief failed")
...
class V5PreTradeAgent:
    ...
    def _log(self, mandate, regime_score, result: V5PreTradeResult):
        ...
        if result.is_veto:
            try:
                ...
                self.logger.exception(f"DB veto save")
    ...
    def override_veto(self, reason: str) -> dict:
        ...
        try:
            ...
            self.logger.exception(f"DB override update")
...
class V5MonitorAgent:
    ...
    def _loop(self):
        while self._running:
            try:
                ...
                self.logger.exception(f"Monitor loop")
    ...
    def _fire_alert(self, triggers, news, vix, nifty):
        ...
        try:
            ...
            self.logger.exception(f"Alert send failed")
...
def _v5_init_tables():
    ...
    try:
        ...
        logger.exception(f"Intelligence table init failed")
        db.rollback()
    finally:
        db.close()
...
class TradeJournal(Base):
    ...
    id = Column(Integer, primary_key=True)
    user_id = Column(Integer, nullable=True, index=True)   # NULL = legacy single-user rows
...
# ── Initialize database — all ORM models now registered with Base.metadata ──────
# This MUST run after all Base subclasses (User, TradeJournal, DailyStats) are defined.
_init_db(os.getenv("DATABASE_URL", "sqlite:///./volguard.db"))

# ── Migrate existing DB — add columns that didn't exist in earlier versions ───────
def _run_migrations():
    for table, col in [
        ("trades",         "user_id INTEGER"),
        ("daily_stats",    "user_id INTEGER"),
        ("users",          "is_admin BOOLEAN DEFAULT 0"),
        ("users",          "last_invalidated_at TIMESTAMP"),
        # admin_audit_log is created by ORM — migration adds future columns here
        ("dynamic_config", "user_id INTEGER"),
    ]:
        try:
            with engine.begin() as conn:
                conn.execute(text(f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS {col}"))
            logger.debug(f"Migration: added column {col} to {table}")
        except Exception as _mig_err:
            # Duplicate column is expected on every restart after first deploy — safe to ignore.
            err_str = str(_mig_err).lower()
            if "duplicate column" in err_str or "already exists" in err_str:
                pass  # expected — column already present
            else:
                logger.warning(f"Migration: unexpected error adding {col} to {table}: {_mig_err}")
    ...
    try:
        with engine.begin() as conn:
            ...
            if result and result[0] == 0:
                ...
                logger.info("✅ First user promoted to admin (migration)")
    except Exception:
        pass
_run_migrations()
...
class TelegramAlertService:
    ...
    async def _process_queue(self):
        while True:
            try:
                ...
                self.logger.exception(f"Telegram Dispatch Error")
    ...
    async def _post_to_api(self, alert: AlertMessage):
        ...
        try:
            ...
            self.logger.exception(f"Telegram Network Error")
...
class AutoTradingEngine:
    ...
    def execute_mandate(self, analysis_data: Dict, mandate: TradingMandate) -> bool:
        ...
        # ── Consecutive losses gate (user-scoped, DB query) ───────────────────
        max_losses = DynamicConfig.get("MAX_CONSECUTIVE_LOSSES")
        with self.db_session_factory() as _gate_db:
            streak = _count_consecutive_losses(_gate_db, user_id=self.active_user_id)
        if streak >= max_losses:
            self.logger.warning(
                f"CONSECUTIVE_LOSS_GATE: {streak} consecutive losses >= limit {max_losses}. "
                f"Trade BLOCKED for user_id={self.active_user_id}. "
                f"Review your last {streak} trades before re-enabling."
            )
            return False
        ...
        for mandate_key, mandate in mandates:
            if mandate and mandate.is_trade_allowed:
                try:
                    ...
                    self.logger.exception(f"Mandate [{mandate_key}] raised exception")
...
class JSONCacheManager:
    ...
    def fetch_and_cache(self, force: bool = False) -> bool:
        ...
        try:
            ...
            self.logger.exception(f"Daily fetch failed")
            with self._lock:
                self._data = {
                    "cache_date": str(today),
                    "fetch_timestamp": now.isoformat(),
    ...
    async def schedule_daily_fetch(self):
        while True:
            try:
                ...
                self.logger.exception(f"Scheduler error")
                await asyncio.sleep(60)
...
class EconomicCalendarEngine:
    ...
    def _fetch_from_tradingview(self, days_ahead: int) -> List[EconomicEvent]:
        ...
        try:
            ...
            self.logger.exception(f"TradingView calendar fetch error")
    ...
    def _fetch_from_investing_com(self, days_ahead: int) -> List[EconomicEvent]:
        ...
        try:
            ...
            self.logger.exception(f"Investing.com fetch error")
...
class VolGuardMarketStreamer:
    ...
    def connect(self, instrument_keys: Optional[List[str]] = None, mode: str = "ltpc"):
        try:
            ...
            self.logger.exception(f"Failed to connect MarketDataStreamerV3")
    ...
    def subscribe(self, instrument_keys: List[str], mode: str):
        with self._lock:
            ...
            try:
                ...
                self.logger.exception(f"Subscribe failed")
                return False
    ...
    def unsubscribe(self, instrument_keys: List[str]):
        with self._lock:
            ...
            try:
                ...
                self.logger.exception(f"Unsubscribe failed")
                return False
    ...
    def change_mode(self, instrument_keys: List[str], mode: str):
        with self._lock:
            ...
            try:
                ...
                self.logger.exception(f"Change mode failed")
                return False
        ...
        with self._lock:
            if self.streamer:
                try:
                    ...
                    self.logger.exception(f"Disconnect error")
    ...
    def _on_sdk_message(self, message):
        try:
            ...
            self.logger.exception(f"Error processing market message")
        ...
        with self._lock:
            for callback in self._callbacks.get(event, []):
                try:
                    ...
                    self.logger.exception(f"Callback error for {event}")
...
class VolGuardPortfolioStreamer:
    ...
    def connect(self, 
        ...
        try:
            ...
            self.logger.exception(f"Failed to connect PortfolioDataStreamer")
            self._dispatch("error", {"type": "connection_error", "message": str(e)})
    ...
    def get_order_status(self, order_id: str) -> Optional[Dict]:
        with self._lock:
            ...
            try:
                ...
                self.logger.exception(f"Failed to fetch order {order_id} status")
                return None
    ...
    def _fetch_order_status_sync(self, order_id: str) -> Optional[Dict]:
        try:
            ...
            self.logger.exception(f"REST order status failed for {order_id}")
    ...
    def _on_sdk_message(self, message):
        try:
            ...
            self.logger.exception(f"Error processing portfolio message")
...
class UpstoxFetcher:
    ...
    def get_live_positions(self) -> Optional[List[Dict]]:
        try:
            ...
            self.logger.exception(f"Portfolio fetch error")
    ...
    def reconcile_positions_with_db(self, db: Session) -> Dict:
        try:
            ...
            self.logger.exception(f"Position reconciliation error")
            return {
                "timestamp": datetime.now().isoformat(),
                "error": str(e),
                "reconciled": False
    ...
    def validate_margin_for_strategy(self, legs: List[OptionLeg]) -> Tuple[bool, float, float]:
        try:
            ...
            self.logger.exception(f"Margin validation error")
    ...
    def get_broker_pnl_for_date(self, target_date: date) -> Optional[float]:
        try:
            ...
            self.logger.exception(f"Broker P&L fetch error")
    ...
    def is_trading_day(self) -> bool:
        try:
            ...
            self.logger.exception(f"Trading day check error")
    ...
    def get_market_status_detailed(self) -> str:
        try:
            ...
            self.logger.exception(f"Market status error")
    ...
    def get_market_holidays(self, days_ahead: int = 30) -> List[date]:
        ...
        try:
            ...
            self.logger.exception(f"Market holidays fetch error")
    ...
    def emergency_exit_all_positions(self) -> Dict:
        try:
            ...
            self.logger.exception(f"Emergency exit failed")
            return {
                "success": False,
                "error": str(e),
                "orders_placed": 0
    ...
    def get_funds(self) -> Optional[float]:
        try:
            ...
            self.logger.exception(f"Fund fetch error")
        return None
    ...
    def get_order_status(self, order_id: str) -> Optional[str]:
        try:
            ...
            self.logger.exception(f"Order status fetch error")
        return None
    ...
    def get_order_details(self, order_id: str) -> Optional[Dict]:
        try:
            ...
            self.logger.exception(f"Order details fetch error")
        return None
    ...
    def history(self, key: str, days: int = 400) -> Optional[pd.DataFrame]:
        try:
            ...
            self.logger.exception(f"History fetch error")
    ...
    def get_expiries(self) -> Tuple[Optional[date], Optional[date], Optional[date], int, List[date]]:
        try:
            ...
            self.logger.exception(f"Expiries fetch error")
    ...
    def chain(self, expiry_date: date) -> Optional[pd.DataFrame]:
        try:
            ...
            if response.status == "success" and response.data:
                ...
                for item in response.data:
                    try:
                        ...
                        self.logger.exception(f"Chain row error")
                        continue
            ...
            self.logger.exception(f"Chain fetch error")
    ...
    def get_greeks(self, instrument_keys: List[str]) -> Dict[str, Dict]:
        try:
            ...
            self.logger.exception(f"Greeks fetch error")
            return {}
...
class UpstoxOrderExecutor:
    ...
    def _get_fill_prices(self, order_ids: List[str]) -> Dict[str, float]:
        ...
        for order_id in order_ids:
            try:
                ...
                self.logger.exception(f"Failed to fetch fill price for order {order_id}")
        return fill_prices
    ...
    def _place_gtt_stop_losses(self, strategy: ConstructedStrategy, filled_orders: Dict[str, Dict]) -> List[str]:
        ...
        for leg in strategy.legs:
            ...
            try:
                ...
                self.logger.exception(f"[GTT-SL] Exception for {leg.strike}")
            ...
            try:
                ...
                self.logger.exception(f"[GTT-TGT] Exception for {leg.strike}")
    ...
    def place_multi_order(self, strategy: ConstructedStrategy, retries_left: int = 3) -> Dict:
        ...
        try:
            ...
            self.logger.exception(f"Multi-order failed")
    ...
    def cancel_gtt_orders(self, gtt_ids: List[str]) -> bool:
        ...
        for gtt_id in gtt_ids:
            try:
                ...
                self.logger.exception(f"Failed to cancel GTT {gtt_id}")
                success = False
        return success
...
class MockExecutor:
    ...
    def exit_position(self, trade, exit_reason: str, current_prices: Dict, db) -> Dict:
        ...
        try:
            ...
            self.logger.exception(f"MockExecutor.exit_position error")
...
class StrategyFactory:
    ...
    def construct_iron_fly(self, expiry_date: date, allocation: float, expiry_type: ExpiryType = ExpiryType.WEEKLY) -> Optional[ConstructedStrategy]:
        try:
            ...
            self.logger.exception(f"Error constructing Iron Fly")
            return None
    ...
    def construct_iron_condor(self, expiry_date: date, allocation: float, expiry_type: ExpiryType = ExpiryType.WEEKLY) -> Optional[ConstructedStrategy]:
        try:
            ...
            self.logger.exception(f"Error constructing Iron Condor")
            return None
    ...
    def construct_protected_straddle(self, expiry_date: date, allocation: float, expiry_type: ExpiryType = ExpiryType.WEEKLY) -> Optional[ConstructedStrategy]:
        try:
            ...
            self.logger.exception(f"Error constructing Protected Straddle")
            return None
    ...
    def construct_protected_strangle(self, expiry_date: date, allocation: float, expiry_type: ExpiryType = ExpiryType.WEEKLY) -> Optional[ConstructedStrategy]:
        try:
            ...
            self.logger.exception(f"Error constructing Protected Strangle")
            return None
    ...
    def construct_bull_put_spread(self, expiry_date: date, allocation: float, expiry_type: ExpiryType = ExpiryType.WEEKLY) -> Optional[ConstructedStrategy]:
        try:
            ...
            self.logger.exception(f"Error constructing Bull Put Spread")
            return None
    ...
    def construct_bear_call_spread(self, expiry_date: date, allocation: float, expiry_type: ExpiryType = ExpiryType.WEEKLY) -> Optional[ConstructedStrategy]:
        try:
            ...
            self.logger.exception(f"Error constructing Bear Call Spread")
            return None
            ...
            try:
                ...
                if should_run:
                    ...
                    try:
                        ...
                        self.logger.exception(f"Analytics calculation failed")
                
                await asyncio.sleep(5)
                
            except Exception as e:
                self.logger.exception(f"Scheduler loop error")
...
# MAX_CONSECUTIVE_LOSSES: 3 (configurable via DynamicConfig)
# Counts closed trades from most-recent backward until a winning trade breaks
# the streak. Compares against config value. User-scoped.
# ============================================================================

def _count_consecutive_losses(db_session, user_id: Optional[int] = None) -> int:
    """
    Return the current consecutive loss streak for a user.
    Queries the last N closed trades ordered by exit_time DESC and counts
    from the most recent backward until a winning trade breaks the streak.
    User-scoped: pass user_id=None for single-user deployments (queries all).
    """
    try:
        q = db_session.query(TradeJournal).filter(
            TradeJournal.status.notin_([TradeStatus.ACTIVE.value, TradeStatus.BUILDING.value]),
            TradeJournal.realized_pnl.isnot(None),
        )
        if user_id is not None:
            q = q.filter(TradeJournal.user_id == user_id)
        recent = q.order_by(TradeJournal.exit_time.desc()).limit(20).all()
        ...
        for trade in recent:
            if (trade.realized_pnl or 0) < 0:
                ...
                break   # first winning trade stops the count
        return streak
    except Exception:
        logger.warning("_count_consecutive_losses query failed — returning 0", exc_info=True)
        return 0
...
# Ported from seed_journal.py. classify_trade_from_obj wraps the core logic
# so it can accept a TradeJournal ORM object directly at every exit point.
# Without this, trade_outcome_class silently stays None on every live trade,
# breaking the coaching layer's SKILL_WIN / LUCKY_WIN breakdown.
# ============================================================================

def _classify_trade_outcome(realized_pnl, theta_pnl, vega_pnl,
                             vov_zscore, regime_score, morning_tone, pretrade_verdict):
    """Core classification logic — identical to seed_journal.py."""
...
def classify_trade_from_obj(trade) -> str:
    """
    Classify a closed trade using its stored entry-context snapshot.
    Accepts a TradeJournal ORM object. Safe to call even if context fields
    were not captured (falls back to SKILL_LOSS).
    """
    try:
        return _classify_trade_outcome(
            realized_pnl      = trade.realized_pnl,
            theta_pnl         = trade.theta_pnl,
            vega_pnl          = trade.vega_pnl,
            vov_zscore        = trade.vov_zscore_at_entry,
            regime_score      = trade.regime_score_at_entry,
            morning_tone      = trade.morning_tone_at_entry,
            pretrade_verdict  = trade.pretrade_verdict_at_entry,
        )
    except Exception:
        logger.warning("classify_trade_from_obj failed — defaulting to SKILL_LOSS", exc_info=True)
        return 'SKILL_LOSS'
...
class PositionMonitor:
    def __init__(self, fetcher: UpstoxFetcher, db_session_factory, analytics_cache: AnalyticsCache, 
        ...
        self._active_user_id: Optional[int] = None  # set from volguard_system.active_user_id
    ...
    async def start_monitoring(self):
        ...
        while self.is_running:
            try:
                ...
                self.logger.exception(f"Monitor error")
                await asyncio.sleep(10)
    ...
    async def check_all_positions(self):
        with self.db_session_factory() as db:
            ...
            try:
                # Scope to active_user_id so one user's losses cannot
                # trigger circuit breaker and close another user's positions.
                q = db.query(TradeJournal).filter(
                    TradeJournal.status == TradeStatus.ACTIVE.value
                )
                if self._active_user_id is not None:
                    q = q.filter(TradeJournal.user_id == self._active_user_id)
                active_trades = q.all()
                ...
                # Trades are written with naive datetime.now() — query boundary must
                # also be naive to avoid bucketing trades into the wrong day at midnight IST.
                today_start = datetime.combine(datetime.now().date(), dt_time.min)
                closed_q = db.query(TradeJournal).filter(
                    TradeJournal.status != TradeStatus.ACTIVE.value,
                    TradeJournal.exit_time >= today_start
                )
                if self._active_user_id is not None:
                    closed_q = closed_q.filter(TradeJournal.user_id == self._active_user_id)
                closed_trades_today = closed_q.all()
                ...
                self.logger.exception(f"Position check error")
...
"""
                ...
                self.logger.exception(f"Alert sending failed")
    
    async def trigger_circuit_breaker(self, db: Session, loop):
        # Scope to the active user — prevents one user's loss from closing
        # another user's positions via a cross-user circuit breaker trigger.
        _cb_query = db.query(TradeJournal).filter(
            TradeJournal.status == TradeStatus.ACTIVE.value
        )
        if self._active_user_id is not None:
            _cb_query = _cb_query.filter(
                (TradeJournal.user_id == self._active_user_id) | (TradeJournal.user_id == None)
            )
        active_trades = _cb_query.all()
...
class VolGuardSystem:
    def __init__(self):
        ...
        # Tracks which user_id currently has an active Upstox token loaded.
        # Set when a user saves their daily token via /api/auth/upstox-token.
        # Used to stamp user_id on trades and filter live positions per user.
    ...
    def _reconcile_gtt_pnl(self, strategy_id: str, order_id: str, fill_price: float) -> None:
        ...
        try:
            ...
            self.logger.exception(f"_reconcile_gtt_pnl error for {strategy_id}")
    ...
    def run_complete_analysis(self) -> Dict:
        try:
            ...
            self.logger.exception(f"Analysis error")
    ...
    def construct_strategy_from_mandate(self, mandate: TradingMandate, 
        ...
        allowed, violations = self.correlation_manager.can_take_position(strategy, user_id=self.active_user_id)
        if not allowed:
    ...
    def execute_strategy(self, strategy: ConstructedStrategy, db: Session, external_metrics=None,
        ...
        # MAX_CONSECUTIVE_LOSSES was in DynamicConfig for months but never
        # checked anywhere. Now enforced: if the last N closed trades are
        # all losses → block new entry, alert via Telegram, point to coach.
        # Behavioural guardrail against revenge trading.
        try:
            max_consec = int(DynamicConfig.get("MAX_CONSECUTIVE_LOSSES", 3))
            recent_closed = (
                db.query(TradeJournal)
                .filter(
                    TradeJournal.status.in_([
                        TradeStatus.CLOSED_PROFIT.value,
                        TradeStatus.CLOSED_STOP_LOSS.value,
                        TradeStatus.CLOSED_EXPIRY_EXIT.value,
                        TradeStatus.CLOSED_CIRCUIT_BREAKER.value,
                        TradeStatus.CLOSED_VETO_EVENT.value,
                        TradeStatus.CLOSED_MANUAL.value,
                    ]),
                    TradeJournal.realized_pnl.isnot(None),
                )
                .order_by(TradeJournal.exit_time.desc())
                .limit(max_consec)
                .all()
            )
            if len(recent_closed) >= max_consec:
                if all((t.realized_pnl or 0) <= 0 for t in recent_closed):
                    total_loss = sum((t.realized_pnl or 0) for t in recent_closed)
                    msg = (
                        f"CONSECUTIVE LOSS BLOCK: last {max_consec} trades all losses "
                        f"(₹{abs(total_loss):,.0f} total). "
                        f"Review your journal before re-entering. "
                        f"Ask your coach: 'What is my pattern in my losing trades?'"
                    )
                    self.logger.warning(msg)
                    if self.alert_service:
                        self.alert_service.send(
                            "Consecutive Loss Block Active",
                            msg,
                            AlertPriority.HIGH,
                            throttle_key="consec_loss_block",
                        )
                    return {
                        "success": False,
                        "message": msg,
                        "consecutive_loss_block": True,
                        "consecutive_losses": max_consec,
                        "total_loss": round(total_loss, 2),
                    }
        except Exception as _cl_err:
            self.logger.warning(f"Consecutive loss check skipped (non-blocking): {_cl_err}")
        # ─────────────────────────────────────────────────────────────────
        ...
        if result['success']:
            ...
            trade = TradeJournal(
                user_id=self.active_user_id,    # stamp which trader placed this
...
class V5JournalCoachAgent:
    ...
    def ask(self, question: str, db, user_id: int = None) -> dict:
        """
        ...
        try:
            # Filter by user_id so each trader sees only their own coaching analysis
            q = db.query(TradeJournal).order_by(TradeJournal.entry_time.desc())
            if user_id is not None:
                q = q.filter(
                    (TradeJournal.user_id == user_id) | (TradeJournal.user_id == None)
                )
...
async def position_reconciliation_job():
    ...
    while True:
        try:
            ...
            logger.exception(f"Position reconciliation error")
...
async def daily_pnl_reconciliation():
    ...
    while True:
        try:
            ...
            logger.exception(f"Daily P&L reconciliation error")
            await asyncio.sleep(3600)
...
async def scheduled_token_refresh():
    ...
    while True:
        try:
            ...
            logger.exception(f"Token refresh scheduler error")
            await asyncio.sleep(60)
...
async def lifespan(app: FastAPI):
            ...
            async def _v5_morning_brief_scheduler():
                while True:
                    try:
                        ...
                        logger.exception(f"Morning Brief scheduler error")
    ...
    # ── 08:47 IST PUSH NOTIFICATION SCHEDULER ────────────────────────────────
    push_task = None
    try:
        from push_notifications import run_morning_brief_scheduler
        push_task = asyncio.create_task(
            run_morning_brief_scheduler(lambda: volguard_system)
        )
        logger.info("📲 Morning push notification scheduler started (08:47 IST daily)")
    except Exception as push_err:
        logger.warning(f"Push notification scheduler not started: {push_err}")
    ...
    if push_task:
        tasks_to_cancel.append(push_task)
...
try:
    ...
    logger.info("Subscription router loaded")
except Exception as _sub_err:
    logger.warning(f"Subscription router not loaded: {_sub_err}")
...
def _update_env_token(new_token: str) -> None:
    ...
    try:
        ...
        logger.exception(f"Failed to persist new token to .env")
...
async def receive_upstox_token(payload: dict, x_webhook_secret: Optional[str] = Header(None, alias="X-Webhook-Secret")):
    ...
    if volguard_system:
        try:
            ...
            logger.exception(f"Failed to hot-swap token in live clients")
...
def get_fii_summary(_user: "User" = Depends(jwt_required)):
    if not volguard_system:
...
def refresh_daily_cache(_user: "User" = Depends(jwt_required)):
    if not volguard_system:
        raise HTTPException(status_code=503, detail="System not initialized")
...
def get_pnl_attribution(db: Session = Depends(get_db), current_user: "User" = Depends(get_current_user)):
    # ── DEMO MODE ──────────────────────────────────────────────────────────────
    if _DEMO_MODE:
    ...
    active_trades = db.query(TradeJournal).filter(
        TradeJournal.status == TradeStatus.ACTIVE.value,
        (TradeJournal.user_id == current_user.id) | (TradeJournal.user_id == None),
    ).all()
    if not active_trades:
...
def list_gtts(db: Session = Depends(get_db), token: str = Depends(verify_token), _active_gtt_user: "User" = Depends(get_current_user)):
    if not volguard_system:
        raise HTTPException(status_code=503, detail="System not initialized")
    
    # Filter to current user's positions only
    active_trades = db.query(TradeJournal).filter(
        TradeJournal.status == TradeStatus.ACTIVE.value,
        (TradeJournal.user_id == _active_gtt_user.id) | (TradeJournal.user_id == None),
    ).all()
    gtts = []
...
def cancel_gtt(
    ...
    # Verify the GTT belongs to a trade owned by the requesting user.
    # GTT IDs are stored as a JSON list in TradeJournal.gtt_order_ids.
    active_trades = db.query(TradeJournal).filter(
        TradeJournal.status == TradeStatus.ACTIVE.value,
        (TradeJournal.user_id == current_user.id) | (TradeJournal.user_id == None),
    ).all()
    ...
    for trade in active_trades:
        if not trade.gtt_order_ids:
            continue
        try:
            ids = json.loads(trade.gtt_order_ids) if isinstance(trade.gtt_order_ids, str) else trade.gtt_order_ids
            if gtt_id in (ids or []):
                owned = True
                break
        except Exception:
    ...
    if not owned:
        raise HTTPException(
            status_code=403,
            detail=f"GTT {gtt_id} not found in your active trades — cancellation denied."
        )
...
async def websocket_subscribe(websocket: WebSocket, token: Optional[str] = None):
    """WebSocket — token is JWT passed as ?token= query param."""
    db = SessionLocal()
    try:
        if not token:
...
async def websocket_market_data(websocket: WebSocket, instrument_key: str, token: Optional[str] = None):
    db = SessionLocal()
    try:
...
async def websocket_portfolio(websocket: WebSocket, token: Optional[str] = None):
    db = SessionLocal()
        ...
        while True:
            ...
            try:
                ...
                try:
                    active_trades = db.query(TradeJournal).filter(
                        TradeJournal.status == TradeStatus.ACTIVE.value,
                        (TradeJournal.user_id == ws_user_id) | (TradeJournal.user_id == None),
                    ).all()
...
def get_market_status(
    _user: "User" = Depends(jwt_required)
):
    if not volguard_system:
        raise HTTPException(status_code=503, detail="System not initialized")
...
def get_dashboard_analytics(
    ...
    try:
        ...
        logger.exception(f"Dashboard analytics error")
    ...
    try:
        ...
        if not analytics:
            try:
                ...
                logger.exception(f"Analytics failed")
...
def get_live_positions(
    ...
    # ── END DEMO MODE ──────────────────────────────────────────────────────────
    try:
        # Filter by user_id — each trader sees only their own positions
        active_trades = db.query(TradeJournal).filter(
            TradeJournal.status == TradeStatus.ACTIVE.value,
            (TradeJournal.user_id == current_user.id) | (TradeJournal.user_id == None),
        ).all()
        ...
        logger.exception(f"Live positions error")
        raise HTTPException(status_code=500, detail=str(e))
...
def get_journal_history(
    ...
    try:
        q = db.query(TradeJournal).filter(
            TradeJournal.status != TradeStatus.ACTIVE.value,
            (TradeJournal.user_id == current_user.id) | (TradeJournal.user_id == None),
        ).order_by(desc(TradeJournal.exit_time)).limit(limit)
        trades = q.all()
        ...
        logger.exception(f"Journal history error")
        raise HTTPException(status_code=500, detail=str(e))
...
def update_system_config(
    ...
    try:
        ...
        # Admins update global config; regular users update only their own overrides
        uid_for_update = None if current_user.is_admin else current_user.id
        changed = DynamicConfig.update(updates, user_id=uid_for_update)

        logger.info(f"Configuration updated via API by user_id={current_user.id} (admin={current_user.is_admin}): {changed}")

        if "AUTO_TRADING" in changed and current_user.is_admin and volguard_system and volguard_system.alert_service:
        ...
        return {
            "success": True,
            "updated": changed,
            "scope": "global" if current_user.is_admin else "personal",
            "current_config": DynamicConfig.to_dict(user_id=current_user.id)
        }

    except Exception as e:
        logger.exception(f"Config update error")
        raise HTTPException(status_code=500, detail=str(e))
...
def get_system_logs(
    ...
    try:
        ...
        logger.exception(f"Logs fetch error")
        raise HTTPException(status_code=500, detail=str(e))
...
def get_current_config(
    ...
    return DynamicConfig.to_dict(user_id=current_user.id)
...
def get_correlation_report(
    ...
    report = volguard_system.correlation_manager.get_correlation_report(user_id=_user.id)
...
def get_capital_structure(
    db: Session = Depends(get_db),
    _cap_user: "User" = Depends(get_current_user),
):
    """
    ...
    # Live deployed capital from active positions — filtered to current user
    active_trades = db.query(TradeJournal).filter(
        TradeJournal.status == TradeStatus.ACTIVE.value,
        (TradeJournal.user_id == _cap_user.id) | (TradeJournal.user_id == None),
    ).all()
...
def v5_get_brief(_user: "User" = Depends(jwt_required)):
...
def v5_global_tone(_user: "User" = Depends(jwt_required)):
...
def v5_macro_snapshot(_user: "User" = Depends(jwt_required)):
...
def v5_news(_user: "User" = Depends(jwt_required)):
...
def v5_veto_log(limit: int = 20, _user: "User" = Depends(jwt_required)):
...
def v5_alerts(limit: int = 10, _user: "User" = Depends(jwt_required)):
...
def reconcile_positions(
    ...
    try:
        ...
        logger.exception(f"Reconcile error")
...
def get_fill_quality_metrics(
    db: Session = Depends(get_db),
    _fill_user: "User" = Depends(get_current_user),
):
    """Fill quality metrics derived from historical completed trades — per user."""
    try:
        completed_trades = db.query(TradeJournal).filter(
            TradeJournal.status != TradeStatus.ACTIVE.value,
            TradeJournal.entry_premium.isnot(None),
            TradeJournal.exit_premium.isnot(None),
            (TradeJournal.user_id == _fill_user.id) | (TradeJournal.user_id == None),
        ...
        logger.exception(f"Fill quality error")
        return {}
...
def journal_coach(
    ...
    """
    # ── Subscription gate — Pro/Team only ────────────────────────────────────
    # Journal Coach calls Claude API on every request (~$0.01-0.03/call).
    # Free tier gets a clear upgrade prompt instead of a silent failure.
    if not current_user.is_admin and current_user.subscription_tier == "free":
        raise HTTPException(
            status_code=402,
            detail={
                "code": "SUBSCRIPTION_REQUIRED",
                "message": "Journal Coach requires a Pro subscription.",
    ...
    result = V5JournalCoachAgent.get().ask(question=request.question.strip(), db=db, user_id=current_user.id)
    if not result["ok"]:
...
def register(payload: RegisterRequest, db: Session = Depends(get_db)):
    """Create a new Fintelligence account."""
    # ── Registration cap ──────────────────────────────────────────────────────
    ...
    if user_count >= max_users:
        raise HTTPException(
            status_code=403,
            detail=f"Registration closed — maximum {max_users} users reached. Contact the admin."
        )
    ...
    return {
        ...
        "has_upstox_token":  False,   # Option B: server never stores broker token
    }
...
def login(payload: LoginRequest, db: Session = Depends(get_db)):
    ...
    # Load this user's config overrides into memory
    DynamicConfig.load_user_overrides(user.id)
    ...
    return {
        ...
        "is_admin":         user.is_admin,
        "subscription_tier": user.subscription_tier,
        # Option B: has_upstox_token is always false from server perspective.
        # The client manages the broker token in sessionStorage only.
        "has_upstox_token": False,
    }
...
def get_me(current_user: User = Depends(get_current_user)):
    ...
    return {
        ...
        # Option B: server never knows if user has a broker token
        "has_upstox_token":  False,
    ...
    return {"users": [
        {
            ...
            "has_upstox":       False,   # Option B: server never stores broker token
            "created_at":       u.created_at.isoformat(),
            "last_login_at":    u.last_login_at.isoformat() if u.last_login_at else None,
        }
        for u in users
    ], "count": len(users), "max_allowed": int(os.getenv("MAX_USERS", "200"))}
...
def update_user(
    ...
    """Admin only — toggle is_active or is_admin for a user."""
    if user_id == admin.id:
        raise HTTPException(status_code=400, detail="Cannot modify your own admin account via this endpoint")
    ...
    db.add(AdminAuditLog(admin_id=admin.id, action="PATCH_USER",
                          target_user_id=user_id, detail=str(changes)))
    db.commit()
    logger.info(f"Admin {admin.id} patched user {user_id}: {changes}")
    return {"success": True, "user_id": user_id, "is_active": user.is_active, "is_admin": user.is_admin}
...
def delete_user(
    ...
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    db.add(AdminAuditLog(admin_id=admin.id, action="DELETE_USER",
                          target_user_id=user_id, detail=f"email={user.email}"))
    db.delete(user)
    db.commit()
...
def revoke_user_tokens(
    ...
    """Admin only — revoke all active JWT tokens for a user immediately."""
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    user.last_invalidated_at = datetime.utcnow()
    db.add(AdminAuditLog(admin_id=admin.id, action="REVOKE_TOKENS", target_user_id=user_id))
    db.commit()
    logger.info(f"Tokens revoked for user_id={user_id} by admin_id={admin.id}")
    return {"success": True, "message": f"All tokens for user {user_id} revoked. They must log in again."}


@app.post("/api/auth/upstox-token")
def activate_upstox_token(
    payload: UpstoxTokenRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """
    Option B — Token-on-Device flow.

    The client sends the broker token once per session so the server can
    initialise the live Upstox SDK for this user's trading session.
    The token is NEVER written to the database — it lives only in memory
    for the duration of the server process, and only on the user's device
    (sessionStorage) on the frontend.

    This is Fintelligence's core trust proposition:
    "Your broker token never leaves your phone."
    """
    token_val = payload.upstox_token.strip()
    if not token_val or len(token_val) < 20:
        raise HTTPException(status_code=400, detail="Token too short — check and try again")

    # ── Activate SDK in-memory only — no DB write ─────────────────────────
    try:
        if volguard_system and hasattr(volguard_system, "fetcher"):
            cfg = upstox_client.Configuration()
            cfg.access_token = token_val
            volguard_system.fetcher.configuration = cfg
            volguard_system.fetcher.api_client = upstox_client.ApiClient(cfg)
            volguard_system.fetcher._reinit_apis()
            volguard_system.active_user_id = current_user.id
            if volguard_system.executor and hasattr(volguard_system.executor, 'monitor'):
                volguard_system.executor.monitor._active_user_id = current_user.id
            logging.getLogger(__name__).info(
                f"Broker session activated for user_id={current_user.id} ({current_user.email}) "
                f"[token NOT persisted — Option B]"
            )
    except Exception:
        logger.exception("SDK activation failed")
        raise HTTPException(status_code=500, detail="Could not activate broker session — check server logs")

    return {
        "success": True,
        "message": "Broker session active — token held in memory only, not stored on server",
    }
...
def health_check(db: Session = Depends(get_db)):
    ...
    if volguard_system and volguard_system.analytics_cache._last_calc_time:
        try:
            ...
            logger.exception(f"Error calculating analytics cache age")
